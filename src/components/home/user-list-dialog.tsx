  ...
  const handleCreateConversation = async () => {
    ...
    try {
      ...
      if (!isGroup) {
        ...
        if(renderedImage){ const postUrl = await generateUploadUrl();

        const result = await fetch(postUrl, {
          method: "POST",
          headers: { "Content-Type": selectedImage!.type },
          body: selectedImage ? selectedImage : null,
        });
   ...
   const usersWithoutGemini = users?.filter((user)=>user.name!=="Gemini AI")
  useEffect(() => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <Input placeholder="Search users by name." onChange={(e)=>setUserSearch(e.target.value)}/>
        {renderedImage && selectedUsers.length > 1 && (
